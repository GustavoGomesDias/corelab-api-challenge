...
describe('Handle get by id API route', () => {
  let server: Server;
  ...
  test('Should return 400 if id is undefined', async () => {
    jest.spyOn(VehicleRepository.prototype, 'getVehicleById').mockImplementationOnce(jest.fn());
    const response = await supertest.get('/vehicle/id/""')
  ...
  test('Should call delete route with corret id', async () => {
    const spy = jest.spyOn(VehicleRepository.prototype, 'getVehicleById').mockImplementationOnce(jest.fn());
    await supertest.get('/vehicle/id/aaaa')
      .expect('Content-Type', /json/);
  ...
  test('Should return 200 if vehicle is finded', async () => {
    jest.spyOn(VehicleRepository.prototype, 'getVehicleById').mockImplementationOnce(jest.fn());
    const response = await supertest.get('/vehicle/id/aaaa')
      .expect('Content-Type', /json/);
